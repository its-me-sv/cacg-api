...
/**
 * returns javascript indentation for given level
 * @param {number} level 
 * @returns 
*/
...
const wSolveFunction = (os, limits, inputs) => {
  ...
  * iterations if the solution exists\n*/\n`;
  ...
  os += `${indent()}for (let ${letter} = ${lb}; ${letter} < 10; ${letter} += 1) {\n`;
  // for 2nd to n-1th letter
  for (let idx = 1; idx < totalLetters - 1; idx += 1) {
    letter = letters[idx];
    lb = limits[letter];
    os += `${indent(idx + 1)}for (let ${letter} = ${lb}; ${letter} < 10; ${letter} += 1) {\n`;
  ...
  for (let word of inputs)
    os += `${indent(idx + 1)}const ${word} = ${summedUpString(word)};\n`;
...
const wMainFunction = os => {
  ...
  os += `${indent()}const start = Date.now();\n`;
  os += `${indent()}// calling the solve function\n`;
  os += `${indent()}solve();\n`;
  os += `${indent()}// total time\n`;
  os += `${indent()}const total_time = (Date.now() - start)/1000;\n`;
...
/**
 * function that generates javascript code
