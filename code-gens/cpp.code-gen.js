...
const wHeader = os => {
  ...
  os += '#include <stdarg.h>\n';
  os += '#include <iostream>\n\n';
  os += '#define cout std::cout\n';
  os += '#define endl std::endl\n';
  os += '\n';
  return os;
};
...
const wSolveFunction = (os, limits, inputs) => {
  ...
  os += `${indent(idx + 1)}cout<<endl<<"${inputs[0]} + ${inputs[1]} = ${inputs[2]}"<<endl;\n`;
  const formatter = "%ld + %ld = %ld\\n";
  os += `${indent(idx + 1)}cout<<${inputs[0]}<<" + "<<${inputs[1]}<<" = "<<${inputs[2]}<<endl;\n`;
  os += `${indent(idx + 1)}cout<<"Iterations: "<<iterations<<endl;\n`;
  ...
  // function end
  os += `${indent()}cout<<"No solution available"<<endl;\n`;
  os += `${indent()}return 0;\n`;
  os += `}\n`;
  os += '\n';
  return os;
...
const wMainFunction = os => {
  ...
  os += `${indent()}cout<<"Time: "<<diff<<" seconds"<<endl;\n`;
  os += `${indent()}return 0;\n`;
  os += `}\n`;
  os += '\n';
  return os;
 ...
 * @returns 
*/
const cppCodeGen = (addend, augend, sum) => {
